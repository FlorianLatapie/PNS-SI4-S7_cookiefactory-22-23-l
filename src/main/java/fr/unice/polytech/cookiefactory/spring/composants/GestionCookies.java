...
public class GestionCookies implements ObtenirCookiesGlobaux, ModifierCookieGlobales {
    ...
    public Cookie getCookieParNom(String nom) {
        return cookieDepot.getTout().stream()
    ...
    public List<Cookie> getCookiesEnAttente() {
        return cookieDepot.getTout().stream().filter(cookie -> cookie.getEtat().equals(ValidationCookie.SOUMIS)).toList();
    }

    @Override
    public List<Cookie> getCookiesValide() {
        return cookieDepot.getTout().stream().filter(cookie -> cookie.getEtat().equals(ValidationCookie.VALIDE)).toList();
    }
